...
const TOOL_LABELS: Record<string, { loading: string; done: string }> = {
  str_replace_editor: { loading: "Writing your design...", done: "Design updated" },
  file_manager:       { loading: "Organizing files...",    done: "Files organized" },
};

function getToolLabel(toolName: string, isDone: boolean): string {
  const entry = TOOL_LABELS[toolName];
  if (!entry) return isDone ? "Done" : "Working...";
  return isDone ? entry.done : entry.loading;
}
                              ...
                              <div key={partIndex} className="inline-flex items-center gap-2 mt-2 px-3 py-1.5 bg-neutral-50 rounded-lg text-xs border border-neutral-200">
                                {isDone ? (
                                  <>
                                    ...
                                    <span className="text-neutral-700">{getToolLabel(tool.toolName, true)}</span>
                                  </>
                                ) : (
                                  <>
                                    <Loader2 className="w-3 h-3 animate-spin text-blue-600" />
                                    <span className="text-neutral-700">{getToolLabel(tool.toolName, false)}</span>
                                  </>
                                )}
                              </div>
                            );
